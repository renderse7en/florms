...
abstract class Element
{
    ...
    /**
     * Add the option key/value by attempting to call the appropriate method,
     * which, if one exists, probably has better or more correct behavior than
     * just blindly adding to the options list. If a method cannot be found
     * for the option name, this will add it to the list anyway.
     *
     * @param string $key
     * @param string $value
     *
    ...
    protected function option($key, $value)
    {
        ...
        return $this->_option($key, $value);
    }

    /**
     * Does the requested option exist?
     *
     * @param string $key
     *
     * @return boolean
